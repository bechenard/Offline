  ...
  void CaloNNDiag::analyze(const art::Event& event)
  {
      ...
      if (vdhits.isValid())
      {
       ...
       for (unsigned int ic=0; ic<CaloHits.size();++ic){
           ...
           CLHEP::Hep3Vector crystalPos  = cal.caloUtil().mu2eToDiskFF(diskId,cal.crystal(hit.crystalID()).position());  //in disk FF frame
       ...
       for (unsigned  ic=0; ic<caloClusters.size();++ic)
       {
          ...
          CLHEP::Hep3Vector seedPos = cal.caloUtil().mu2eToDiskFF(cluster.diskID(),cal.crystal(seedHit.crystalID()).position());
          ...
          for (unsigned i=0;i< eDepMCs.size();++i)
          {
              ...
              if (vdMapEntry != vdMap.end())
              {
                 ...
                 simPos = cal.caloUtil().mu2eToDiskFF(cluster.diskID(), vdMapEntry->second->position());
              }
           ...
           for (auto iter=vdhits->begin(), ie=vdhits->end(); iter!=ie; ++iter)
             {
               ...
               CLHEP::Hep3Vector VDPos = cal.caloUtil().mu2eToTracker(hit.position());
