...
namespace mu2e {
  ...
  class LHEGun : public art::EDProducer {
     ...
     public:
        ...
        struct Config 
        {
            ...
            fhicl::Atom<double>        timeOffset     { Name("timeOffset"),    Comment("Time Ofset to add to particles"),0.0 }; 
  ...
  void LHEGun::fill(GenParticleCollection& output )
  {
     ...
     double time = stop.t + timeOffset_;
     if (doHistograms_) hTime_->Fill(time);

     reader_.readNextEvent();

     //return an empty vector if we don't have anymore events - should we issue a warning? 
	 ...
	 output.emplace_back(PDGCode::type(pdgId),genId_,pos,fourmom,time);

	 if (doHistograms_)
	 {
	    hPdgId_->Fill(pdgId);
	    hEnergy_->Fill(energy);
            htheta_->Fill(p3.cosTheta());
	    hZ_->Fill(pos.z());
