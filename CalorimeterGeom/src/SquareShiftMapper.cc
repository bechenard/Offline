...
namespace mu2e {
  ...
  {
    step_.push_back({ 1, 1}); //right
    step_.push_back({ 0, 1}); //down right
    step_.push_back({-1, 0}); //down left
    step_.push_back({-1,-1}); //left
    step_.push_back({ 0,-1}); //up left
    step_.push_back({ 1, 0}); //up right
  }
  ...
  std::vector<unsigned> SquareShiftMapper::neighbors(unsigned thisIndex, unsigned level) const
  {
    ...
    for (unsigned i=0; i<step_.size(); ++i)
      for (unsigned j=0; j<level; ++j) {
        current.add(step_[i]);
        result.push_back(index(current));
      }
