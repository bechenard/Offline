...
namespace mu2e {
    ...
    DiskCalorimeterMaker::DiskCalorimeterMaker(SimpleConfig const& config, double solenoidOffset)
    {
       ...
       calo_->G4Info_.set("nSiPMPerCrystal",        CaloConst::_nSiPMPerCrystal);                          //number of SiPM per crystal
       ...
       calo_->G4Info_.set("pipeTorRadius",          getVDouble(config,"pipeTorRadius", calo_->G4Info_.getInt("nPipes")) ); //Tor radius of cooling pipes and other
       calo_->G4Info_.set("largeTorPhi",            getVDouble(config,"largeTorPhi",   calo_->G4Info_.getInt("nPipes")) ); //cooling pipe characteristics
       calo_->G4Info_.set("smallTorPhi",            getVDouble(config,"smallTorPhi",   calo_->G4Info_.getInt("nPipes")) ); //(see constructDiskCalorimeter)
       calo_->G4Info_.set("yposition",              getVDouble(config,"yposition",     calo_->G4Info_.getInt("nPipes")) );
       calo_->G4Info_.set("straightEndPhi",         getVDouble(config,"straightEndPhi",calo_->G4Info_.getInt("nPipes")) );
       ...
       calo_->G4Info_.set("cratePhiAngles",         getVDouble(config,"cratePhiAngles",calo_->G4Info_.getInt("nCrates")) ); //phi angle of the crates
       ...
       calo_->util_.trackerCenter(CLHEP::Hep3Vector(xOrigin,0,zTrackerCenter));                   //origin of the Tracker mother volume in Mu2e
    ...
    {
        double vdThickness           = calo_->G4Info_.getDouble("vdThickness");
        double caloMotherZ0          = calo_->G4Info_.getDouble("caloMotherZ0");
        double caloMotherZ1          = calo_->G4Info_.getDouble("caloMotherZ1");
        double crystalHalfXY         = calo_->G4Info_.getDouble("crystalXYLength")/2.0;
        double wrapperHalfThick      = calo_->G4Info_.getDouble("wrapperThickness")/2.0;
        double crystalHalfZLength    = calo_->G4Info_.getDouble("crystalZLength")/2.0;
        double crystalCapHalfZLength = calo_->G4Info_.getDouble("crystalCapZLength")/2.0;
        double crystalCellRadius     = crystalHalfXY + 2.0*wrapperHalfThick;
        double innerDiskRadius       = calo_->G4Info_.getDouble("diskInAlRingRIn");
        double innerCrysRadius       = calo_->G4Info_.getDouble("diskCrystalRIn");
        double outerCrysRadius       = calo_->G4Info_.getDouble("diskCrystalROut");
        double outerCaseRadius       = calo_->G4Info_.getDouble("diskCaseRingROut");
        double diskOutRailROut       = calo_->G4Info_.getDouble("diskOutRailROut");
        auto   shimStepsInRowId      = calo_->G4Info_.getVInt("shimStepsInRowId");
        auto   shimStepsOutRowId     = calo_->G4Info_.getVInt("shimStepsOutRowId");
        double FPCarbonThick         = calo_->G4Info_.getDouble("FPCarbonZLength");
        double FPFoamThick           = calo_->G4Info_.getDouble("FPFoamZLength");
        double FPCoolPipeRadius      = calo_->G4Info_.getDouble("FPCoolPipeRadius");
        double FPpipeRadius          = calo_->G4Info_.getDouble("pipeRadius");
        double diskCaseHalfZLength   = crystalHalfZLength + crystalCapHalfZLength;
        double BPHoleHalfZ           = calo_->G4Info_.getDouble("BPHoleZLength")/2.0;
        double FEEBoxHalfZ           = calo_->G4Info_.getDouble("FEEZLength")/2.0;
        double FEEBoxThick           = calo_->G4Info_.getDouble("FEEBoxThickness");
        double BPPipeRadiusHigh      = calo_->G4Info_.getDouble("BPPipeRadiusHigh");
        double BPPipeHalfZOffset     = calo_->G4Info_.getDouble("BPPipeZOffset")/2.0;
        double crateZLength          = calo_->G4Info_.getDouble("crateZLength");
        double crateFShieldThick     = calo_->G4Info_.getDouble("crateFShieldThickness");
        double crateFShieldDeltaZ    = calo_->G4Info_.getDouble("crateFShieldDeltaZ");
        double FEBToDiskZOffset      = calo_->G4Info_.getDouble("FEBToDiskZOffset");
        double motherHalfZ           = (caloMotherZ1 - caloMotherZ0)/2.0;
        const auto& diskCrystalFile  = calo_->G4Info_.getString("diskCrystalFile");
        const auto& caloOrigin       = calo_->G4Info_.getHepVec("caloOrigin");
        ...
        for (int idisk=0; idisk<CaloConst::_nDisk; ++idisk) {
           double separation    = calo_->G4Info_.getVDouble("diskZMotherShift").at(idisk);
           double angleZ        = 0;
    ...
    void DiskCalorimeterMaker::checkIt(void)
    {
        ...
        double diskRin       = calo_->G4Info_.getDouble("diskInAlRingRIn");
        double diskRout      = calo_->G4Info_.getDouble("diskCaseRingROut");
        double outerRingROut = calo_->G4Info_.getDouble("diskOutRailROut");
        if (diskRin > diskRout)
              throw cet::exception("DiskCaloGeom") << "calorimeter.diskInnerRingIn > calorimeter.diskOuterRingOut \n";

        if (outerRingROut > calo_->G4Info_.getDouble("caloDiskRadiusOut"))
              throw cet::exception("DiskCaloGeom") << "calorimeter outer radius larger than calorimeter mother \n";

        if (diskRin < calo_->G4Info_.getDouble("caloDiskRadiusIn"))
        ...
        if (calo_->G4Info_.getDouble("BPHoleXLength") > calo_->G4Info_.getDouble("crystalXYLength") ||
            calo_->G4Info_.getDouble("BPHoleYLength") > calo_->G4Info_.getDouble("crystalXYLength") )
              throw cet::exception("DiskCaloGeom") << "calorimeter backplate hole greater than crystal dimensions in X or Y \n";

        if (calo_->G4Info_.getDouble("readoutXLength") > calo_->G4Info_.getDouble("BPHoleXLength")  ||
            calo_->G4Info_.getDouble("readoutYLength") > calo_->G4Info_.getDouble("BPHoleYLength"))
              throw cet::exception("DiskCaloGeom") << "calorimeter readout larger than hole in X or Y \n";

        if (calo_->G4Info_.getDouble("readoutZLength") > calo_->G4Info_.getDouble("BPHoleZLength"))
              throw cet::exception("DiskCaloGeom") << "calorimeter readout too thick to fit in hole \n";

        if (calo_->G4Info_.getDouble("FEEXLength") > calo_->G4Info_.getDouble("BPHoleXLength")/calo_->G4Info_.getInt("nSiPMPerCrystal"))
              throw cet::exception("DiskCaloGeom") << "calorimeter FEE box does not fit in X direction \n";

        if (calo_->G4Info_.getDouble("FEEYLength") > calo_->G4Info_.getDouble("crystalXYLength")-2*calo_->G4Info_.getDouble("FEEBoxThickness"))
              throw cet::exception("DiskCaloGeom") << "calorimeter FEE box does not fit in Y direction \n";


        //Check pipes
        for (int i=0;i<calo_->G4Info().getInt("nPipes");++i){
           double pipeTorRadius     = calo_->G4Info_.getVDouble("pipeTorRadius").at(i);
           double pipeRadius        = calo_->G4Info_.getDouble("pipeRadius");
           double radiusIn          = calo_->G4Info_.getDouble("FPInnerRadius");
           double radiusOut         = calo_->G4Info_.getDouble("FPOuterRadius");
           double foamZLength       = calo_->G4Info_.getDouble("FPFoamZLength");
           double carbonThick       = calo_->G4Info_.getDouble("FPCarbonZLength");
           double foamThick         = calo_->G4Info_.getDouble("FPFoamZLength");
           double coolPipeTorRadius = calo_->G4Info_.getDouble("FPCoolPipeTorRadius");
           double coolPipeRadius    = calo_->G4Info_.getDouble("FPCoolPipeRadius");
        ...
        int nBoards           = calo_->G4Info_.getInt("nBoards");
        double radiatorDY     = calo_->G4Info_.getDouble("radiatorThickness")/2.0;
        double activeStripDY  = calo_->G4Info_.getDouble("activeStripThickness")/2.0;
        double passiveStripDY = calo_->G4Info_.getDouble("passiveStripThickness")/2.0;
        double crateXLength   = calo_->G4Info_.getDouble("crateXLength");
        double crateYLength   = calo_->G4Info_.getDouble("crateYLength");
        ...
        if (calo_->G4Info_.getDouble("crateFShieldYLength") > crateYLength)
              throw cet::exception("DiskCaloGeom") << "calorimeter FEB front shile too long in Y direction\n";

        if (calo_->G4Info_.getDouble("crateSThickness") > crateXLength)
              throw cet::exception("DiskCaloGeom") << "calorimeter FEB crate side too thick\n";

        if (calo_->G4Info_.getDouble("crateTThickness") > crateYLength )
