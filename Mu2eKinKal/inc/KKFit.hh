...
namespace mu2e {
  ...
  template <class KTRAJ> std::shared_ptr<SensorLine> KKFit<KTRAJ>::caloAxis(CaloCluster const& cluster, Calorimeter const& calo) const {
    ...
    CLHEP::Hep3Vector cog = calo.caloUtil().mu2eToTracker(calo.caloUtil().diskFFToMu2e( cluster.diskID(), cluster.cog3Vector()));
    ...
    VEC3 ffcog(cog);
    double lcrystal = calo.G4Info().getDouble("crystalZLength"); // text-keyed lookup is very inefficient FIXME!
  ...
  template <class KTRAJ> void KKFit<KTRAJ>::addCaloHit(Calorimeter const& calo, KKTRK& kktrk, CCHandle cchandle, KKCALOHITCOL& hits) const {
    double crystalLength = calo.G4Info().getDouble("crystalZLength");
    auto const& ptraj = kktrk.fitTraj();
    ...
    for(unsigned idisk=0; idisk < 2; ++idisk){
      auto ffpos = calo.caloUtil().mu2eToTracker(calo.disk(idisk).diskInfo().frontFaceCenter());
  ...
  template <class KTRAJ> KalSeed KKFit<KTRAJ>::createSeed(KKTRK const& kktrk, TrkFitFlag const& seedflag, Calorimeter const& calo, Tracker const& nominalTracker) const {
    ...
    if(kktrk.caloHits().size() > 0){
      ...
      double backz = calo.caloUtil().mu2eToTracker(calo.disk(calohit->caloCluster()->diskID()).diskInfo().backFaceCenter()).z();
