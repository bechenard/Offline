...
namespace mu2e {
  ...
  void CalLineTimePeakFinder::beginRun(art::Run& ) {
    ...
    calo_d0_offset_ = calorimeter_->caloUtil().mu2eToTracker(calorimeter_->caloUtil().diskToMu2e(0, CLHEP::Hep3Vector(0., 0., 0.))).z();
    calo_d1_offset_ = calorimeter_->caloUtil().mu2eToTracker(calorimeter_->caloUtil().diskToMu2e(1, CLHEP::Hep3Vector(0., 0., 0.))).z();

    // get the target position in the tracker system
    target_pos_ = calorimeter_->caloUtil().mu2eToTracker(stopping_target_->centerInMu2e());
