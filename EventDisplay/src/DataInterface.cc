...
//Envelope
  ...
  if(geom->hasElement<mu2e::DiskCalorimeter>())
  {
    ...
    double diskCaseDZLength      = calo->G4Info().getDouble("diskCaseZLength")/2.0;
    double diskRadiusIn          = calo->G4Info().getDouble("caloDiskRadiusIn");
    double diskRadiusOut         = calo->G4Info().getDouble("caloDiskRadiusOut");

    double FPCarbonDZ               = calo->G4Info().getDouble("FPCarbonZLength")/2.0;
    double FPFoamDZ                 = calo->G4Info().getDouble("FPFoamZLength")/2.0;
    double FPCoolPipeRadius         = calo->G4Info().getDouble("FPCoolPipeRadius");
    double pipeRadius               = calo->G4Info().getDouble("pipeRadius");
    double frontPanelHalfThick      = (2.0*FPCarbonDZ+2.0*FPFoamDZ-pipeRadius+FPCoolPipeRadius)/2.0;

    double crystalDXY            = calo->G4Info().getDouble("crystalXYLength")/2.0;
    double crystalDZ             = calo->G4Info().getDouble("crystalZLength")/2.0;
    double crystalFrameDZ        = calo->G4Info().getDouble("crystalCapZLength")/2.0;
    double wrapperHalfThick      = calo->G4Info().getDouble("wrapperThickness")/2.0;
    double wrapperDXY            = crystalDXY + 2.0*wrapperHalfThick;
    double wrapperDZ             = crystalDZ + 2.0*crystalFrameDZ;

    double FEEDZ                = calo->G4Info().getDouble("FEEZLength")/2.0;
    double FEEBoxThickness      = calo->G4Info().getDouble("FEEBoxThickness");
    double FEEBoxDZ             = FEEDZ + 2*FEEBoxThickness;
    double BPPipeRadiusHigh     = calo->G4Info().getDouble("BPPipeRadiusHigh");
    double BPPipeDZOffset       = calo->G4Info().getDouble("BPPipeZOffset")/2.0;
    double BPFEEDZ              = FEEBoxDZ + BPPipeDZOffset + BPPipeRadiusHigh;
    double holeDZ               = calo->G4Info().getDouble("BPHoleZLength")/2.0;
