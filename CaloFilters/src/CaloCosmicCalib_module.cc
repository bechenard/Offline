...
namespace mu2e
{
  ...
  bool CaloCosmicCalib::beginRun(art::Run & run){
    ...
    _calogeom    = ch.get();
    _crystalsize = _calogeom->G4Info().getDouble("crystalXYLength") +  2.*_calogeom->G4Info().getDouble("wrapperThickness");
    ...
    for(auto icl = clcol->begin();icl != clcol->end(); ++icl) {
      ...
      if( (energy >= _minenergy) &&
        ...
        for( int icry=0; icry< clsize; icry++){
          ...
          CLHEP::Hep3Vector crystalpos = _calogeom->caloUtil().mu2eToDiskFF(DiskId,_calogeom->crystal(_cryid).position());
