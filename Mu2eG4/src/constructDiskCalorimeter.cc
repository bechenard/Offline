...
namespace mu2e {
  ...
  VolumeInfo constructDiskCalorimeter(const VolumeInfo& mother, const SimpleConfig& config)
  {
    ...
    const double caloDiskRadiusIn   = cal.G4Info().getDouble("caloDiskRadiusIn");
    const double caloDiskRadiusOut  = cal.G4Info().getDouble("caloDiskRadiusOut");
    const double caloFEBRadiusOut   = cal.G4Info().getDouble("caloFEBRadiusOut");
    const double mother_z0          = cal.G4Info().getDouble("caloMotherZ0");
    const double mother_z1          = cal.G4Info().getDouble("caloMotherZ1");
    const double mother_zlength     = mother_z1-mother_z0;
    const double mother_zCenter     = (mother_z1+mother_z0)/2.0;
    const double FEBOffset          = cal.G4Info().getDouble("FEBToDiskZOffset");
    const auto   FEBPhiMinMax       = calcFEBPhiRange(cal);
    const bool   hasCrates          = cal.G4Info().getBool("hasCrates");
  ...
  VolumeInfo caloBuildDisk(const SimpleConfig& config, unsigned idisk)
  {
    ...
    const bool hasFrontPanel   = cal.G4Info().getBool("hasFrontPanel");
    const bool hasBackPanel    = cal.G4Info().getBool("hasBackPanel");
    const double vdThickness   = cal.G4Info().getDouble("vdThickness");
    const double R0disk        = cal.G4Info().getDouble("caloDiskRadiusIn");
    const double R1disk        = cal.G4Info().getDouble("caloDiskRadiusOut");
  ...
  VolumeInfo caloBuildFrontPlate(const SimpleConfig& config, unsigned idisk)
  {
    ...
    const double FPInnerRadius               = cal.G4Info().getDouble("FPInnerRadius");
    const double FPOuterRadius               = cal.G4Info().getDouble("FPOuterRadius");
    const double FPCarbonDZ                  = cal.G4Info().getDouble("FPCarbonZLength")/2.0;
    const double FPFoamDZ                    = cal.G4Info().getDouble("FPFoamZLength")/2.0;
    const double FPCoolPipeTorRadius         = cal.G4Info().getDouble("FPCoolPipeTorRadius");
    const double FPCoolPipeRadius            = cal.G4Info().getDouble("FPCoolPipeRadius");
    const double FPCoolPipeThickness         = cal.G4Info().getDouble("FPCoolPipeThickness");
    const double FPCoolPipeRadiusIn          = FPCoolPipeRadius-FPCoolPipeThickness;

    const int nPipes                         = cal.G4Info().getInt    ("nPipes");
    const double pipeRadius                  = cal.G4Info().getDouble ("pipeRadius");
    const double pipeThickness               = cal.G4Info().getDouble ("pipeThickness");
    const double pipeInitSeparation          = cal.G4Info().getDouble ("pipeInitSeparation");
    const std::vector<double> pipeTorRadius  = cal.G4Info().getVDouble("pipeTorRadius");
    const std::vector<double> largeTorPhi    = cal.G4Info().getVDouble("largeTorPhi");
    const std::vector<double> smallTorPhi    = cal.G4Info().getVDouble("smallTorPhi");
    const std::vector<double> yposition      = cal.G4Info().getVDouble("yposition");
    const std::vector<double> straightEndPhi = cal.G4Info().getVDouble("straightEndPhi");
    const double radSmTor                    = cal.G4Info().getDouble ("radSmTor");
    const double xsmall                      = cal.G4Info().getDouble ("xsmall");
    const double xdistance                   = cal.G4Info().getDouble ("xdistance");
  ...
  VolumeInfo caloBuildCase(const SimpleConfig& config, unsigned idisk)
  {
    ...
    const double vdThickness              = cal.G4Info().getDouble("vdThickness");
    const double crystalDXY               = cal.G4Info().getDouble("crystalXYLength")/2.0;
    const double crystalDZ                = cal.G4Info().getDouble("crystalZLength")/2.0;
    const double crystalCapDZ             = cal.G4Info().getDouble("crystalCapZLength")/2.0;
    const double wrapperHalfThick         = cal.G4Info().getDouble("wrapperThickness")/2.0;
    const double wrapperDXY               = crystalDXY + 2*wrapperHalfThick;
    const double wrapperDZ                = crystalDZ+crystalCapDZ;
    const std::vector<int> caphriCystalId = cal.G4Info().getVInt("caphriCrystalId");

    const double diskCaseDZ               = wrapperDZ;
    const double diskInAlRingRIn          = cal.G4Info().getDouble("diskInAlRingRIn");
    const double diskInAlRingDZ           = cal.G4Info().getDouble("diskInAlRingZLength")/2.0;
    const double diskInCFRingRIn          = cal.G4Info().getDouble("diskInCFRingRIn");
    const double diskInCFRingROut         = cal.G4Info().getDouble("diskInCFRingROut");
    const double diskCaseRingROut         = cal.G4Info().getDouble("diskCaseRingROut");
    const double diskOutRailROut          = cal.G4Info().getDouble("diskOutRailROut") - vdThickness;
    const double diskOutRailDZ            = cal.G4Info().getDouble("diskOutRailZLength")/2.0;

    const double FPCoolPipeRadius         = cal.G4Info().getDouble("FPCoolPipeRadius");
    const double FPCoolPipeThickness      = cal.G4Info().getDouble("FPCoolPipeThickness");
    const double coolPipeZpos             = (diskCaseDZ - 2*FPCoolPipeRadius - 2.0*diskOutRailDZ)/2.0 + FPCoolPipeRadius;

    const std::vector<double> stepsInX    = cal.G4Info().getVDouble("stepsInsideX");
    const std::vector<double> stepsInY    = cal.G4Info().getVDouble("stepsInsideY");
    const std::vector<double> stepsOutX   = cal.G4Info().getVDouble("stepsOutsideX");
    const std::vector<double> stepsOutY   = cal.G4Info().getVDouble("stepsOutsideY");
    const double diskStepThickness        = cal.G4Info().getDouble ("diskStepThickness");
  ...
  VolumeInfo caloBuildBackPlate(const SimpleConfig& config, unsigned idisk)
  {
    ...
    const double BPInnerRadius       = cal.G4Info().getDouble("FPInnerRadius"); //same as front plate
    const double BPOuterRadius       = cal.G4Info().getDouble("BPOuterRadius");
    const double diskCrystalRIn      = cal.G4Info().getDouble("diskCrystalRIn");
    const double diskCrystalROut     = cal.G4Info().getDouble("diskCrystalROut");

    const double crystalDXY          = cal.G4Info().getDouble("crystalXYLength")/2.0;
    const double wrapperDXY          = crystalDXY + cal.G4Info().getDouble("wrapperThickness");
    const double RODX                = cal.G4Info().getDouble("readoutXLength")/2.0;
    const double RODY                = cal.G4Info().getDouble("readoutYLength")/2.0;
    const double RODZ                = cal.G4Info().getDouble("readoutZLength")/2.0;
    const double holeDX              = cal.G4Info().getDouble("BPHoleXLength")/2.0;
    const double holeDY              = cal.G4Info().getDouble("BPHoleYLength")/2.0;
    const double holeDZ              = cal.G4Info().getDouble("BPHoleZLength")/2.0;
    const double stripDY             = wrapperDXY-holeDY-1.0;
    const double stripDZ             = cal.G4Info().getDouble("BPStripThickness")/2.0;
    const double FEEDX               = cal.G4Info().getDouble("FEEXLength")/2.0;
    const double FEEDY               = cal.G4Info().getDouble("FEEYLength")/2.0;
    const double FEEDZ               = cal.G4Info().getDouble("FEEZLength")/2.0;

    const double FEEBoxThickness     = cal.G4Info().getDouble("FEEBoxThickness");
    ...
    const double BPPipeRadiusHigh    = cal.G4Info().getDouble("BPPipeRadiusHigh");
    const double BPPipeRadiusLow     = cal.G4Info().getDouble("BPPipeRadiusLow");
    const double BPPipeThickness     = cal.G4Info().getDouble("BPPipeThickness");
    ...
    const double BPPipeDZOffset      = cal.G4Info().getDouble("BPPipeZOffset")/2.0;
  ...
  VolumeInfo caloBuildFEB(const SimpleConfig& config, unsigned idisk)
  {
    ...
    const int    nCrates           = cal.G4Info().getInt("nCrates");
    const double vdThickness       = cal.G4Info().getDouble("vdThickness");
    const double caloDiskRadiusOut = cal.G4Info().getDouble("caloDiskRadiusOut");
    const auto   cratePhiAngle     = cal.G4Info().getVDouble("cratePhiAngles");
  ...
  VolumeInfo caloBuildCrate(const SimpleConfig& config, unsigned idisk)
  {
    ...
    const int nBoards                = cal.G4Info().getInt   ("nBoards");
    const double crateDX             = cal.G4Info().getDouble("crateXLength")/2.0;
    const double crateDY             = cal.G4Info().getDouble("crateYLength")/2.0;
    const double crateDZ             = cal.G4Info().getDouble("crateZLength")/2.0;
    const double crateFShieldDisp    = cal.G4Info().getDouble("crateFShieldDeltaZ");
    const double crateFShieldThick   = cal.G4Info().getDouble("crateFShieldThickness");
    const double crateBottomThick    = cal.G4Info().getDouble("crateBShieldThickness");
    const double crateBottomLength   = cal.G4Info().getDouble("crateBShieldLength");
    const double crateTopThick       = cal.G4Info().getDouble("crateTThickness");
    const double crateSideThick      = cal.G4Info().getDouble("crateSThickness");
    const double crateFShieldDY      = cal.G4Info().getDouble("crateFShieldYLength")/2.0;

    const double radiatorDY          = cal.G4Info().getDouble("radiatorThickness")/2.0;
    const double radiatorDZ          = cal.G4Info().getDouble("radiatorZLength")/2.0;
    const double activeStripDY       = cal.G4Info().getDouble("activeStripThickness")/2.0;
    const double passiveStripDY      = cal.G4Info().getDouble("passiveStripThickness")/2.0;
  ...
  {
    G4double crateXLength  = cal.G4Info().getDouble("crateXLength");
    G4double crateRin      = cal.G4Info().getDouble("caloDiskRadiusOut");
    auto cratePhis         = cal.G4Info().getVDouble("cratePhiAngles");
