...
/*
       for (unsigned int ic=0; ic<CaloHits.size();++ic)
       {
           ...
           CLHEP::Hep3Vector crystalPos  = cal.caloUtil().mu2eToDiskFF(diskId,cal.crystal(hit.id()).position());  //in disk FF frame
           ...
           for (int ip=0; ip<nPartInside;++ip)
           {
             ...
             CLHEP::Hep3Vector hitSimPos = cal.caloUtil().mu2eToDiskFF(diskId,hitSim.position().at(ip)); //in disk FF frame
