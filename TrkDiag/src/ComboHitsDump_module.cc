...
namespace mu2e
{
  ...
  class ComboHitsDump : public art::EDAnalyzer {
    ...
    private:
      ...
      std::vector<float> _hitTime,_hiteDep,_hitDirQ;
      std::vector<XYZVectorF> _hitPos,_hitDir,_hitMCmom;
  ...
  void ComboHitsDump::beginJob() {
    ...
    _hitDiag->Branch("pos",     &_hitPos     );
    _hitDiag->Branch("dir",     &_hitDir     );
    ...
    if (_mcdiag){
      ...
      _hitDiag->Branch("mcmom ", &_hitMCmom   );
    }
  ...
  void ComboHitsDump::analyze(const art::Event& event ) {
    ...
    // Ntuple - all hits
    //----------------------------------
      _hitPos.clear();  _hitDir.clear(); _hitDirQ.clear();
      _hitTime.clear(); _hiteDep.clear();_hitMCmom.clear();
      _hitnch.clear();  _hitnsh.clear(); _hitPdg.clear();
      _hitproc.clear(); _hitid.clear();  _bkghinfo.clear();

      for (size_t ich=0;ich<_chcol->size();++ich){
        const auto& ch   = _chcol->at(ich);
        //const auto& shf  = _bkghitcol->at(ich).flag();

        _hitPos .push_back(ch.pos());
        _hitDir .push_back(ch.hDir());
        ...
        if (_mcdiag) {
          ...
          _hitMCmom .push_back(mcdigi.earlyStrawGasStep()->momentum());
        }
