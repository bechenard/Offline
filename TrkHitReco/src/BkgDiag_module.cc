...
#include "Offline/TrkHitReco/inc/StrawHitInfo.hh"
// art
...
namespace mu2e
{
  ...
  class BkgDiag : public art::EDAnalyzer {
    public:
      ...
      struct Config {
        ...
        fhicl::Atom<art::InputTag>    ComboHitCollectionInit{	Name("ComboHitCollectionInit"),    Comment("ComboHit collection name") };
     };
    ...
    private:
      ...
      art::ProductToken<ComboHitCollection> _chTokenInit;
      ...
      const ComboHitCollection* _chcolInit;
      ...
      std::vector<StrawHitInfo> _bkghinfo;
      std::vector<int> _chindices;
      std::vector<float> _chmommc;

      Int_t              _nhits;
      std::vector<int>   _hitPdg,_hitproc,_hitnch,_hitnsh,_hitid;
      std::vector<float> _hitPosX,_hitPosY,_hitPosZ,_hitTime,_hiteDep,_hitDirX,_hitDirY,_hitDirZ,_hitDirQ,_hitMCmomX,_hitMCmomY,_hitMCmomZ;
  };
  ...
  BkgDiag::BkgDiag(const art::EDAnalyzer::Table<Config>& config) :
    ...
    _chTokenInit{ consumes<ComboHitCollection>(config().ComboHitCollectionInit() ) },
  ...
  void BkgDiag::beginJob() {
    ...
    _bcdiag->Branch("chmomm",&_chmommc);
  ...
  void BkgDiag::analyze(const art::Event& event ) {
    ...
    for (size_t ich=0;ich<_chcolInit->size();++ich){
      _hitPosX.push_back(_chcolInit->at(ich).pos().x());
      _hitPosY.push_back(_chcolInit->at(ich).pos().y());
      _hitPosZ.push_back(_chcolInit->at(ich).pos().z());
      _hitDirX.push_back(_chcolInit->at(ich).hDir().x());
      _hitDirY.push_back(_chcolInit->at(ich).hDir().y());
      _hitDirZ.push_back(_chcolInit->at(ich).hDir().z());
      _hitDirQ.push_back(_chcolInit->at(ich).qual());
      _hitTime.push_back(_chcolInit->at(ich).time());
      _hiteDep.push_back(_chcolInit->at(ich).energyDep());
      _hitnch.push_back(_chcolInit->at(ich).nCombo());
      _hitnsh.push_back(_chcolInit->at(ich).nStrawHits());
      art::Ptr<SimParticle> spp; // main SimParticle for this
      if(_mcdiag){
        std::vector<StrawDigiIndex> dids;
        _chcolInit->fillStrawDigiIndices(ich,dids);
    ...
    for (size_t ibkg=0;ibkg<_bkgccol->size();++ibkg){
      ...
      for(auto const& ich : cluster.hits()){
        ...
        _chmommc.push_back(bkghinfo._mcmom);
  ...
  bool BkgDiag::findData(const art::Event& evt){
    ...
    _chcol = chH.product();
    auto chHInit = evt.getValidHandle(_chTokenInit);
    _chcolInit = chHInit.product();
